...
export function EpureSvg({ points, selectedId = null, onSelect }: EpureSvgProps) {
  ...
  // Нативні touch-події: надійний пінч і зсув на телефонах.
  useEffect(() => {
    const svg = svgRef.current
    if (!svg) return
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      if (touchPoints.current.size === 1) {
        ...
        pinch.current = { dist: Math.hypot(a.x - b.x, a.y - b.y) || 1, k: viewRef.current.k, tx: viewRef.current.tx, ty: viewRef.current.ty }
      }
    }
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (last) {
        const [p] = [...touchPoints.current.values()]
        if (p) {
          commitPan(p.x - last.px, p.y - last.py)
          drag.current = { px: p.x, py: p.y }
        }
      }
    }
